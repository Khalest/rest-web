...
export class TodosController {
  ...
  getTodoById = async (req: Request, res: Response) => {
    const { id } = req.params;
    if (!id) return res.status(HTTP_STATUS_BAD_REQUEST).json({ message: "Id is required" });
  ...
  createTodo = async (req: Request, res: Response) => {
    ...
    if (error) return res.status(HTTP_STATUS_BAD_REQUEST).json({ message: error });
    const todo = await prisma.todo.create({ data: todoDto!.toPrimitives() });
